...
Optional:
  ...
  --heatmap-max-labels 80      # max number of tick labels to show on each axis (auto subsample)

Notes:
- OneHotEncoder 参数已做版本兼容（支持老版本'sparse'与新版本'sparse_output'）。
"""
...
def make_labeled_heatmap(corr_df: pd.DataFrame, out_path: str, max_labels: int = 80) -> None:
    """
    Draw a Pearson correlation heatmap with feature-name tick labels.
    If features > max_labels, subsample labels evenly on both axes.
    """
    if corr_df.shape[0] == 0:
        return
    features = corr_df.columns.tolist()
    n = len(features)
    fig = plt.figure(figsize=(max(8, min(0.12*n, 24)), max(6, min(0.12*n, 24))))  # 自适应画布
    ax = plt.gca()
    im = ax.imshow(corr_df.values, aspect="auto", interpolation="nearest")
    ...
    ax.set_xticks(idxs)
    ax.set_yticks(idxs)
    ax.set_xticklabels([features[i] for i in idxs], rotation=90, ha="center", va="top", fontsize=8)
    ax.set_yticklabels([features[i] for i in idxs], fontsize=8)
...
def _build_compatible_ohe():
    """
    Build OneHotEncoder compatible with both old and new scikit-learn:
...
def main():
    ...
    ap.add_argument("--heatmap-max-labels", type=int, default=80, help="Max tick labels shown per axis on heatmap")
    args = ap.parse_args()
    ...
    # Heatmap with text tick labels
    heatmap_path = None
    if args.heatmap or len(numeric_cols) >= 2:
        corr_mat_df = feature_df[numeric_cols].corr() if len(numeric_cols) >= 2 else pd.DataFrame()
        heatmap_path = os.path.join(args.out_dir, "correlation_heatmap.png")
        if not corr_mat_df.empty:
            make_labeled_heatmap(corr_mat_df, heatmap_path, max_labels=args.heatmap_max_labels)
