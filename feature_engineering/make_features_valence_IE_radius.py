...
# -*- coding: utf-8 -*-
"""
make_features_valence_IE_radius.py

变更点（符合你的新规则）
- 电离能：只对金属 M 计算“与价态一致的累计电离能” T0_M__IE_sum_valence = sum(IE1..IE_valence)
- 离子半径：只对金属 M，按 (名义价态 from 化学计量, 配位CN from st1 -> {4:IV, 6:VI}) 精确选择一条 ionic/crystal 半径（Å）
- 卤素 H：不导出 IE 与 r_ion 两类；其它元素物性照常保留
- 其他保持：elements 数值列（M/H 两套）、元素比值、pairwise 的差/比/加权平均、T1/T2

依赖：mendeleev>=1.1.0, pandas, numpy（可选：pymatgen 用于 formula 解析与 T1/T2）
"""
...
CN_MAP = {4: "IV", 6: "VI"}  # st1 -> CN 映射，可按需扩展
...
def main():
    ...
    # IE1..IE_n 宽表：为普适性，取到 10 阶；后续按“价态 n”做部分求和
    ies = fetch_ionization_energies(degree=list(range(1, 11))).reset_index()  # IE1..IE10
    # 两套离子/晶体半径矩阵 -> 长表
